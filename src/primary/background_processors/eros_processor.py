"""
Eros background processor for Huntarr.

This module handles background processing for Eros instances, including
...
class ErosProcessor(BaseProcessor):
    """Handles Eros background processing"""
    
    def __init__(self, hunting_manager):
        """
        Initialize the Eros processor.
    ...
    def _process_implementation(self, stop_event) -> None:
        ...
        try:
            ...
            if not eros_config:
                self.log_error("No configuration found for Eros, cannot process hunting")
                return
            ...
            self.log_error(f"Error in Eros hunting process: {str(e)}")
